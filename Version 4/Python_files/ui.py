...
class add:
    def __init__(self):
        ...
        components = {'Firmware','Driver','Both','All','FW_All'}
...
def save_info():
    ...
    for elem in obj:
        ...
        print(spp_name)
        if fill.get()=="" or cd_drive.get()=="" or output_fill.get()=="":
...
def select_output():
    ...
    output_fill.set(output_directory.replace('/','\\\\').strip('\\\\'))
...
select_spp_button = Button(main,text='Select SPP iso',command=select_spp, font=('Helvetica', 16, 'bold'),fg="#ffffff",bg="#05386b",activebackground="#0538bb",activeforeground="#ffffff").place(x=25,y=100)
select_custombaseline_button = Button(main,text='Select custom baseline',command=custom_baseline, font=('Helvetica', 16, 'bold'),fg="#ffffff",bg="#05386b",activebackground="#0538bb",activeforeground="#ffffff").place(x=450,y=100)
select_output_button = Button(main,text='Select Output Path',command=select_output, font=('Helvetica', 16, 'bold'),fg="#ffffff",bg="#05386b",activebackground="#0538bb",activeforeground="#ffffff").place(x=1400,y=100)

select_cd=Entry(text='E',textvariable=cd_drive,width=30).place(x=1120,y=115)
select_label=Entry(textvariable=fill,width=30,state=DISABLED).place(x=200,y=110)
select_custombaseline_label= Entry(textvariable=custom_fill,width=30,state=DISABLED).place(x=720,y=110)
select_output_label = Entry(textvariable=output_fill,width=30,state=DISABLED).place(x=1620,y=110)

Label(text="*", font=('Helvetica', 16, 'bold'),fg="#000000",bg="#659DBD").place(x=1300,y=115)
cd_label = Label(text="CD drive name:", font=('Helvetica', 16, 'bold'),fg="#000000",bg="#659DBD").place(x=950,y=110)
Label(text="*", font=('Helvetica', 16, 'bold'),fg="#000000",bg="#659DBD").place(x=370,y=110)
Label(text="*", font=('Helvetica', 16, 'bold'),fg="#000000",bg="#659DBD").place(x=1800,y=110)
